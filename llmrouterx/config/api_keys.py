...
class APIKeyDatabase:
    ...
    def create_key(
        ...
        expires_in: Optional[str] = None,  # e.g., "24h", "7d", "30d"
    ) -> tuple[str, APIKey]:
        ...
        with self._transaction() as conn:
            cursor = conn.execute("""
                INSERT INTO api_keys (key_hash, prefix, name, scopes, created_at, expires_at, last_used_at, revoked)
                VALUES (?, ?, ?, ?, ?, ?, ?, 0)
            """, (key_hash, prefix, name, scopes_json, now.isoformat(),
                  expires_at.isoformat() if expires_at else None, None))
    ...
    def get_key(self, key_hash: str) -> Optional[APIKey]:
...
_db: Optional[APIKeyDatabase] = None
_db_lock = threading.Lock()


def get_api_key_db(db_path: Optional[str] = None) -> APIKeyDatabase:
...
def validate_api_key(full_key: str, db_path: Optional[str] = None) -> Optional[APIKey]:
...
def resolve_api_key(
    ...
    if client.get("api_key_db"):
        ...
        if client.get("api_key"):
            ...
            raise KeyResolutionError(f"Invalid or revoked API key")
